...
// loading cartridge from allocator because we need to chekc file size and allocate buffer
pub fn loadCartRomAlloc(allocator: std.mem.Allocator) ![]u8 {
    var cwd = std.fs.cwd();
    ...
    var names_buf: [4096]u8 = undefined;
    var fba = std.heap.FixedBufferAllocator.init(&names_buf);
    const allocator_names = fba.allocator();
    ...
    while (try it.next()) |entry| {
        ...
        const name_copy = try allocator_names.dupe(u8, entry.name);
        rom_names[rom_count] = name_copy;
        rom_count += 1;
    }

    std.debug.print("Available ROMs in ./roms for Cartridge:\n", .{});
    var i: usize = 0;
    while (i < rom_count) : (i += 1) {
        std.debug.print("  {d}) {s}\n", .{ i, rom_names[i] });
    }
    ...
    const full_path = try std.fs.path.join(allocator_names, &.{ "../roms", rom_name });
    var file = try cwd.openFile(full_path, .{ .mode = .read_only });
    defer file.close();

    // Determine file size, allocate exactly that much
    const stat = try file.stat();
    const rom_size = @as(usize, @intCast(stat.size));
